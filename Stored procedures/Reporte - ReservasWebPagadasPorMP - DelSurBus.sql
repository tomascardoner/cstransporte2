	...
	BEGIN
		...
		SELECT DISTINCT vd.IDViajeDetalle, vd.FechaHora, vd.IDRuta, vd.Orden, dbo.udf_GetEntidadApellidoYNombre(p.Apellido, p.Nombre) AS ApellidoNombre, vd.FacturaNumero, pa.codigoPago AS CodigoPago, pa.monto AS ImportePago
			FROM ViajeDetalle AS vd
				...
				INNER JOIN CSTransporte_Web_DelSurBus.dbo.PagoReserva AS pr ON r.idReserva = pr.idReserva
				INNER JOIN CSTransporte_Web_DelSurBus.dbo.Pago AS pa ON pr.idPago = pa.idPago
...
GO
GRANT SELECT ON CSTransporte_Web_DelSurBus.dbo.PagoReserva TO cstransporte
GO
GRANT SELECT ON CSTransporte_Web_DelSurBus.dbo.Pago TO cstransporte
GO