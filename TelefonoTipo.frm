...
Begin VB.Form frmTelefonoTipo 
   Caption         =   "Tipos de Teléfono"
   ClientHeight    =   5955
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   7410
   Icon            =   "TelefonoTipo.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MDIChild        =   -1  'True
   ...
   End
End
Attribute VB_Name = "frmTelefonoTipo"
...
Public Sub FillListView(ByVal IDTelefonoTipo As Long)
    ...
    If IDTelefonoTipo = 0 Then
        ...
        End If
    Else
        KeySave = KEY_STRINGER & IDTelefonoTipo
    End If
    
    SQL_Where = " WHERE TelefonoTipo.IDTelefonoTipo <> " & pParametro.TelefonoTipo_ID_Otro
    
    If cboFilterActivo.ListIndex > 0 Then
        SQL_Where = SQL_Where & IIf(SQL_Where = "", " WHERE ", " AND ") & "TelefonoTipo.Activo = " & IIf(cboFilterActivo.ListIndex = 1, 1, 0)
    End If
    ...
    recData.Source = "SELECT TelefonoTipo.IDTelefonoTipo, TelefonoTipo.Nombre, TelefonoTipo.Activo FROM TelefonoTipo" & SQL_Where
    recData.Open , pDatabase.Connection, adOpenForwardOnly, adLockReadOnly, adCmdText
    
    With recData
        If Not .EOF Then
            Do While Not .EOF
                Set ListItem = lvwData.ListItems.Add(, KEY_STRINGER & .Fields("IDTelefonoTipo").Value, .Fields("Nombre").Value)
...
ErrorHandler:
    ShowErrorMessage "Forms.TelefonoTipo.FillListView", "Error al obtener la lista de Tipos de Teléfono."
End Sub
...
Private Sub Form_Load()
    ...
    pParametro.GetCoolBarSettings "TelefonoTipo", cbrMain
    pParametro.GetListViewSettings "TelefonoTipo", lvwData
    lvwData.ColumnHeaders(lvwData.SortKey + 1).Icon = lvwData.SortOrder + 1
    tlbPin.Buttons("PIN").Value = pParametro.Usuario_LeerNumero("TelefonoTipo_Pin", tlbPin.Buttons("PIN").Value)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    pParametro.SaveCoolBarSettings "TelefonoTipo", cbrMain
    pParametro.SaveListViewSettings "TelefonoTipo", lvwData
    pParametro.Usuario_GuardarNumero "TelefonoTipo_Pin", tlbPin.Buttons("PIN").Value
End Sub
...
Private Sub tlbMain_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Select Case Button.Key
        Case "NEW"
            If pCPermiso.GotPermission(PERMISO_TELEFONO_TIPO_ADD) Then
                Screen.MousePointer = vbHourglass
                
                Set TelefonoTipo = New TelefonoTipo
                frmTelefonoTipoPropiedad.LoadDataAndShow Me, TelefonoTipo
                Set TelefonoTipo = Nothing
                
                Screen.MousePointer = vbDefault
            End If
        Case "PROPERTIES"
            If pCPermiso.GotPermission(PERMISO_TELEFONO_TIPO_MODIFY) Then
                ...
                Set TelefonoTipo = New TelefonoTipo
                TelefonoTipo.IDTelefonoTipo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
            ...
            End If
        Case "DELETE"
            If pCPermiso.GotPermission(PERMISO_TELEFONO_TIPO_DELETE) Then
                If lvwData.SelectedItem Is Nothing Then
                    MsgBox "No hay ningún Item seleccionado.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
                End If
                If MsgBox("¿Desea eliminar el Tipo de Teléfono seleccionado?", vbQuestion + vbYesNo, App.Title) = vbYes Then
                    Set TelefonoTipo = New TelefonoTipo
                    TelefonoTipo.IDTelefonoTipo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                    If TelefonoTipo.Load() Then
                        Call TelefonoTipo.Delete
                    End If
                    Set TelefonoTipo = Nothing
        ...
        Case "SELECT"
            ...
            If FormIndex >= 0 Then
                ...
                Set TelefonoTipo = New TelefonoTipo
                TelefonoTipo.IDTelefonoTipo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                If Not TelefonoTipo.Load() Then
                    Set TelefonoTipo = Nothing
                    Exit Sub
                End If
                If Not TelefonoTipo.Activo Then
                ...
                End If
                Screen.MousePointer = vbHourglass
                Forms(FormIndex).TelefonoTipoSelected TelefonoTipo.IDTelefonoTipo
                Set TelefonoTipo = Nothing
