...
Begin VB.Form frmCuentaCorrienteCaja 
   Caption         =   "Cajas de Cuenta Corriente"
   ClientHeight    =   5955
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   7410
   Icon            =   "CuentaCorrienteCaja.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MDIChild        =   -1  'True
   ...
   End
End
Attribute VB_Name = "frmCuentaCorrienteCaja"
...
Public Sub FillListView(ByVal IDCuentaCorrienteCaja As Long)
    ...
    If IDCuentaCorrienteCaja = 0 Then
        If Not lvwData.SelectedItem Is Nothing Then
            KeySave = lvwData.SelectedItem.Key
        End If
    Else
        KeySave = KEY_STRINGER & IDCuentaCorrienteCaja
    End If
    
    SQL_Where = ""
    
    If cboFilterActivo.ListIndex > 0 Then
        SQL_Where = SQL_Where & IIf(SQL_Where = "", " WHERE ", " AND ") & "CuentaCorrienteCaja.Activo = " & IIf(cboFilterActivo.ListIndex = 1, 1, 0)
    End If
    ...
    recData.Source = "SELECT CuentaCorrienteCaja.IDCuentaCorrienteCaja, CuentaCorrienteCaja.Nombre, CuentaCorrienteCaja.Activo FROM CuentaCorrienteCaja" & SQL_Where
    recData.Open , pDatabase.Connection, adOpenForwardOnly, adLockReadOnly, adCmdText
    
    With recData
        If Not .EOF Then
            Do While Not .EOF
                Set ListItem = lvwData.ListItems.Add(, KEY_STRINGER & .Fields("IDCuentaCorrienteCaja").Value, .Fields("Nombre").Value)
...
ErrorHandler:
    ShowErrorMessage "Forms.CuentaCorrienteCaja.FillListView", "Error al obtener la lista de Cajas de Cuenta Corriente."
End Sub
...
Private Sub Form_Load()
    ...
    pParametro.GetCoolBarSettings "CuentaCorrienteCaja", cbrMain
    pParametro.GetListViewSettings "CuentaCorrienteCaja", lvwData
    lvwData.ColumnHeaders(lvwData.SortKey + 1).Icon = lvwData.SortOrder + 1
    tlbPin.Buttons("PIN").Value = pParametro.Usuario_LeerNumero("CuentaCorrienteCaja_Pin", tlbPin.Buttons("PIN").Value)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    pParametro.SaveCoolBarSettings "CuentaCorrienteCaja", cbrMain
    pParametro.SaveListViewSettings "CuentaCorrienteCaja", lvwData
    pParametro.Usuario_GuardarNumero "CuentaCorrienteCaja_Pin", tlbPin.Buttons("PIN").Value
End Sub
...
Private Sub tlbMain_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Select Case Button.Key
        Case "NEW"
            If pCPermiso.GotPermission(PERMISO_CUENTA_CORRIENTE_CAJA_ADD) Then
                Screen.MousePointer = vbHourglass
                
                Set CuentaCorrienteCaja = New CuentaCorrienteCaja
                frmCuentaCorrienteCajaPropiedad.LoadDataAndShow Me, CuentaCorrienteCaja
                Set CuentaCorrienteCaja = Nothing
                
                Screen.MousePointer = vbDefault
            End If
        Case "PROPERTIES"
            If pCPermiso.GotPermission(PERMISO_CUENTA_CORRIENTE_CAJA_MODIFY) Then
                ...
                Set CuentaCorrienteCaja = New CuentaCorrienteCaja
                CuentaCorrienteCaja.IDCuentaCorrienteCaja = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
            ...
            End If
        Case "DELETE"
            If pCPermiso.GotPermission(PERMISO_CUENTA_CORRIENTE_CAJA_DELETE) Then
                If lvwData.SelectedItem Is Nothing Then
                    MsgBox "No hay ningún Item seleccionado.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
                End If
                If MsgBox("¿Desea eliminar el CuentaCorrienteCaja seleccionado?", vbQuestion + vbYesNo, App.Title) = vbYes Then
                    Set CuentaCorrienteCaja = New CuentaCorrienteCaja
                    CuentaCorrienteCaja.IDCuentaCorrienteCaja = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                    If CuentaCorrienteCaja.Load() Then
                        Call CuentaCorrienteCaja.Delete
                    End If
                    Set CuentaCorrienteCaja = Nothing
        ...
        Case "SELECT"
            ...
            If FormIndex >= 0 Then
                ...
                Set CuentaCorrienteCaja = New CuentaCorrienteCaja
                CuentaCorrienteCaja.IDCuentaCorrienteCaja = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                If Not CuentaCorrienteCaja.Load() Then
                    Set CuentaCorrienteCaja = Nothing
                    Exit Sub
                End If
                If Not CuentaCorrienteCaja.Activo Then
                ...
                End If
                Screen.MousePointer = vbHourglass
                Forms(FormIndex).CuentaCorrienteCajaSelected CuentaCorrienteCaja.IDCuentaCorrienteCaja
                Set CuentaCorrienteCaja = Nothing
