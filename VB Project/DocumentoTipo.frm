...
Begin VB.Form frmDocumentoTipo 
   Caption         =   "Tipos de Documento"
   ClientHeight    =   5955
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   7410
   Icon            =   "DocumentoTipo.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MDIChild        =   -1  'True
   ...
   End
End
Attribute VB_Name = "frmDocumentoTipo"
...
Public Sub FillListView(ByVal IDDocumentoTipo As String)
    ...
    If IDDocumentoTipo = "" Then
        If Not lvwData.SelectedItem Is Nothing Then
            KeySave = lvwData.SelectedItem.Key
        End If
    Else
        KeySave = KEY_STRINGER & IDDocumentoTipo
    End If
    
    SQL_Where = ""
    
    If cboFilterActivo.ListIndex > 0 Then
        SQL_Where = SQL_Where & IIf(SQL_Where = "", " WHERE ", " AND ") & "DocumentoTipo.Activo = " & IIf(cboFilterActivo.ListIndex = 1, 1, 0)
    End If
    ...
    recData.Source = "SELECT DocumentoTipo.IDDocumentoTipo, DocumentoTipo.Nombre, DocumentoTipo.Activo FROM DocumentoTipo" & SQL_Where
    recData.Open , pDatabase.Connection, adOpenForwardOnly, adLockReadOnly, adCmdText
    
    With recData
        If Not .EOF Then
            Do While Not .EOF
                Set ListItem = lvwData.ListItems.Add(, KEY_STRINGER & .Fields("IDDocumentoTipo").Value, .Fields("Nombre").Value)
...
ErrorHandler:
    ShowErrorMessage "Forms.DocumentoTipo.FillListView", "Error al obtener la lista de Tipos de Documento."
End Sub
...
Private Sub Form_Load()
    ...
    pParametro.GetCoolBarSettings "DocumentoTipo", cbrMain
    pParametro.GetListViewSettings "DocumentoTipo", lvwData
    lvwData.ColumnHeaders(lvwData.SortKey + 1).Icon = lvwData.SortOrder + 1
    tlbPin.Buttons("PIN").Value = pParametro.Usuario_LeerNumero("DocumentoTipo_Pin", tlbPin.Buttons("PIN").Value)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    pParametro.SaveCoolBarSettings "DocumentoTipo", cbrMain
    pParametro.SaveListViewSettings "DocumentoTipo", lvwData
    pParametro.Usuario_GuardarNumero "DocumentoTipo_Pin", tlbPin.Buttons("PIN").Value
End Sub
...
Private Sub tlbMain_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Dim DocumentoTipo As DocumentoTipo
    
    Select Case Button.Key
        Case "NEW"
            If pCPermiso.GotPermission(PERMISO_DOCUMENTO_TIPO_ADD) Then
                Screen.MousePointer = vbHourglass
                
                Set DocumentoTipo = New DocumentoTipo
                frmDocumentoTipoPropiedad.LoadDataAndShow Me, DocumentoTipo
                Set DocumentoTipo = Nothing
                
                Screen.MousePointer = vbDefault
            End If
        Case "PROPERTIES"
            If pCPermiso.GotPermission(PERMISO_DOCUMENTO_TIPO_MODIFY) Then
                ...
                Set DocumentoTipo = New DocumentoTipo
                DocumentoTipo.IDDocumentoTipo = Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1)
                If DocumentoTipo.Load() Then
                    frmDocumentoTipoPropiedad.LoadDataAndShow Me, DocumentoTipo
                End If
                Set DocumentoTipo = Nothing
            ...
            End If
        Case "DELETE"
            If pCPermiso.GotPermission(PERMISO_DOCUMENTO_TIPO_DELETE) Then
                If lvwData.SelectedItem Is Nothing Then
                    MsgBox "No hay ningún Item seleccionado.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
                End If
                If MsgBox("¿Desea eliminar el Tipo de Documento seleccionado?", vbQuestion + vbYesNo, App.Title) = vbYes Then
                    Set DocumentoTipo = New DocumentoTipo
                    DocumentoTipo.IDDocumentoTipo = Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1)
                    If DocumentoTipo.Load() Then
                        Call DocumentoTipo.Delete
                    End If
                    Set DocumentoTipo = Nothing
        ...
        Case "SELECT"
            ...
            If FormIndex >= 0 Then
                ...
                Set DocumentoTipo = New DocumentoTipo
                DocumentoTipo.IDDocumentoTipo = Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1)
                If Not DocumentoTipo.Load() Then
                    Set DocumentoTipo = Nothing
                    Exit Sub
                End If
                If Not DocumentoTipo.Activo Then
                ...
                End If
                Screen.MousePointer = vbHourglass
                Forms(FormIndex).DocumentoTipoSelected DocumentoTipo.IDDocumentoTipo
                Set DocumentoTipo = Nothing
