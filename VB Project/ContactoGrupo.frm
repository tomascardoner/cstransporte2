...
Begin VB.Form frmContactoGrupo 
   Caption         =   "Grupos de Contactos"
   ClientHeight    =   5955
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   7410
   Icon            =   "ContactoGrupo.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MDIChild        =   -1  'True
   ...
   End
End
Attribute VB_Name = "frmContactoGrupo"
...
Public Sub FillListView(ByVal IDContactoGrupo As Long)
    ...
    If IDContactoGrupo = 0 Then
        If Not lvwData.SelectedItem Is Nothing Then
            KeySave = lvwData.SelectedItem.Key
        End If
    Else
        KeySave = KEY_STRINGER & IDContactoGrupo
    End If
    
    SQL_Where = ""
    
    If cboFilterActivo.ListIndex > 0 Then
        SQL_Where = SQL_Where & IIf(SQL_Where = "", " WHERE ", " AND ") & "ContactoGrupo.Activo = " & IIf(cboFilterActivo.ListIndex = 1, 1, 0)
    End If
    ...
    recData.Source = "SELECT ContactoGrupo.IDContactoGrupo, ContactoGrupo.Nombre, ContactoGrupo.Activo FROM ContactoGrupo" & SQL_Where
    recData.Open , pDatabase.Connection, adOpenForwardOnly, adLockReadOnly, adCmdText
    
    With recData
        If Not .EOF Then
            Do While Not .EOF
                Set ListItem = lvwData.ListItems.Add(, KEY_STRINGER & .Fields("IDContactoGrupo").Value, .Fields("Nombre").Value)
...
ErrorHandler:
    ShowErrorMessage "Forms.ContactoGrupo.FillListView", "Error al obtener la lista de Grupos de Contactos."
End Sub
...
Private Sub Form_Load()
    ...
    pParametro.GetCoolBarSettings "ContactoGrupo", cbrMain
    pParametro.GetListViewSettings "ContactoGrupo", lvwData
    lvwData.ColumnHeaders(lvwData.SortKey + 1).Icon = lvwData.SortOrder + 1
    tlbPin.Buttons("PIN").Value = pParametro.Usuario_LeerNumero("ContactoGrupo_Pin", tlbPin.Buttons("PIN").Value)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    pParametro.SaveCoolBarSettings "ContactoGrupo", cbrMain
    pParametro.SaveListViewSettings "ContactoGrupo", lvwData
    pParametro.Usuario_GuardarNumero "ContactoGrupo_Pin", tlbPin.Buttons("PIN").Value
End Sub
...
Private Sub tlbMain_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Dim ContactoGrupo As ContactoGrupo
    
    Select Case Button.Key
        Case "NEW"
            If pCPermiso.GotPermission(PERMISO_CONTACTO_GRUPO_ADD) Then
                Screen.MousePointer = vbHourglass
                
                Set ContactoGrupo = New ContactoGrupo
                frmContactoGrupoPropiedad.LoadDataAndShow Me, ContactoGrupo
                Set ContactoGrupo = Nothing
                
                Screen.MousePointer = vbDefault
            End If
        Case "PROPERTIES"
            If pCPermiso.GotPermission(PERMISO_CONTACTO_GRUPO_MODIFY) Then
                ...
                Set ContactoGrupo = New ContactoGrupo
                ContactoGrupo.IDContactoGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                If ContactoGrupo.Load() Then
                    frmContactoGrupoPropiedad.LoadDataAndShow Me, ContactoGrupo
                End If
                Set ContactoGrupo = Nothing
            ...
            End If
        Case "DELETE"
            If pCPermiso.GotPermission(PERMISO_CONTACTO_GRUPO_DELETE) Then
                If lvwData.SelectedItem Is Nothing Then
                    MsgBox "No hay ningún Item seleccionado.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
        ...
        Case "SELECT"
            ...
            If FormIndex >= 0 Then
                ...
                Set ContactoGrupo = New ContactoGrupo
                ContactoGrupo.IDContactoGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                If Not ContactoGrupo.Load() Then
                    Set ContactoGrupo = Nothing
                    Exit Sub
                End If
                If Not ContactoGrupo.Activo Then
                ...
                End If
                Screen.MousePointer = vbHourglass
                Forms(FormIndex).ContactoGrupoSelected ContactoGrupo.IDContactoGrupo
                Set ContactoGrupo = Nothing
