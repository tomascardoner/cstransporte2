...
Public Sub FillListView(ByVal IDAlarma As Long)
    ...
    If IDAlarma = 0 Then
        ...
        End If
    Else
        KeySave = KEY_STRINGER & IDAlarma
    End If
    
    SQL_Where = ""
    
    If cboFilterActivo.ListIndex > 0 Then
        SQL_Where = SQL_Where & IIf(SQL_Where = "", " WHERE ", " AND ") & "Alarma.Activo = " & IIf(cboFilterActivo.ListIndex = 1, 1, 0)
    End If
    ...
    recData.Source = "SELECT Alarma.IDAlarma, Alarma.Nombre, Alarma.Activo FROM Alarma" & SQL_Where
    recData.Open , pDatabase.Connection, adOpenForwardOnly, adLockReadOnly, adCmdText
    
    With recData
        If Not .EOF Then
            Do While Not .EOF
                Set ListItem = lvwData.ListItems.Add(, KEY_STRINGER & .Fields("IDAlarma").Value, .Fields("Nombre").Value)
...
ErrorHandler:
    ShowErrorMessage "Forms.Alarma.FillListView", "Error al obtener la lista de las Alarmas Generales."
End Sub
...
Private Sub Form_Load()
    ...
    pParametro.GetCoolBarSettings "Alarma", cbrMain
    pParametro.GetListViewSettings "Alarma", lvwData
    lvwData.ColumnHeaders(lvwData.SortKey + 1).Icon = lvwData.SortOrder + 1
    tlbPin.Buttons("PIN").Value = pParametro.Usuario_LeerNumero("Alarma_Pin", tlbPin.Buttons("PIN").Value)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    pParametro.SaveCoolBarSettings "Alarma", cbrMain
    pParametro.SaveListViewSettings "Alarma", lvwData
    pParametro.Usuario_GuardarNumero "Alarma_Pin", tlbPin.Buttons("PIN").Value
    Set frmAlarma = Nothing
End Sub
...
Private Sub tlbMain_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Select Case Button.Key
        Case "NEW"
            If pCPermiso.GotPermission(PERMISO_ALARMA_ADD) Then
                Screen.MousePointer = vbHourglass
                frmAlarmaPropiedad.LoadDataAndShow Me, Alarma
                Screen.MousePointer = vbDefault
            End If
        Case "PROPERTIES"
            If pCPermiso.GotPermission(PERMISO_ALARMA_MODIFY) Then
                ...
                Alarma.IDAlarma = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                If Not Alarma.Load() Then
                    Set Alarma = Nothing
                    lvwData.SetFocus
                    Exit Sub
                End If
            ...
            End If
        Case "DELETE"
            If pCPermiso.GotPermission(PERMISO_ALARMA_DELETE) Then
                If lvwData.SelectedItem Is Nothing Then
                    MsgBox "No hay ningún Item seleccionado.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
                End If
                If MsgBox("¿Desea eliminar la Alarma General seleccionada?", vbExclamation + vbYesNo, App.Title) = vbYes Then
        ...
        Case "SELECT"
            ...
            If FormIndex >= 0 Then
                ...
                End If
                Screen.MousePointer = vbHourglass
                Forms(FormIndex).AlarmaSelected Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
