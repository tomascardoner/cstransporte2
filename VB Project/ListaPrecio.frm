...
Public Sub FillListView(ByVal IDLugar As Long)
    ...
    If pCPermiso.ListaPrecioWhere <> "" Then
        SQL_Where = SQL_Where & IIf(SQL_Where = "", " WHERE ", " AND ") & Replace(pCPermiso.ListaPrecioWhere, "%TABLENAME%", "ListaPrecio")
    End If
    ...
    recData.Source = "SELECT IDListaPrecio, Nombre, Activo FROM ListaPrecio" & SQL_Where
    recData.Open , pDatabase.Connection, adOpenForwardOnly, adLockReadOnly, adCmdText
    
    With recData
        If Not .EOF Then
            Do While Not .EOF
                Set ListItem = lvwData.ListItems.Add(, KEY_STRINGER & .Fields("IDListaPrecio").Value, .Fields("Nombre").Value)
...
ErrorHandler:
    ShowErrorMessage "Forms.ListaPrecio.FillListView", "Error al obtener las Listas de Precios"
End Sub
...
Private Sub Form_Load()
    ...
    pParametro.GetCoolBarSettings "ListaPrecio", cbrMain
    pParametro.GetListViewSettings "ListaPrecio", lvwData
    lvwData.ColumnHeaders(lvwData.SortKey + 1).Icon = lvwData.SortOrder + 1
    tlbPin.Buttons("PIN").Value = pParametro.Usuario_LeerNumero("ListaPrecio_Pin", tlbPin.Buttons("PIN").Value)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    pParametro.SaveCoolBarSettings "ListaPrecio", cbrMain
    pParametro.SaveListViewSettings "ListaPrecio", lvwData
    pParametro.Usuario_GuardarNumero "ListaPrecio_Pin", tlbPin.Buttons("PIN").Value
End Sub
...
Private Sub tlbMain_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Select Case Button.Key
        Case "NEW"
            If pCPermiso.GotPermission(PERMISO_LISTA_PRECIO_ADD) Then
                Screen.MousePointer = vbHourglass
                
                Set ListaPrecio = New ListaPrecio
                frmListaPrecioPropiedad.LoadDataAndShow Me, ListaPrecio
                Set ListaPrecio = Nothing
                
                Screen.MousePointer = vbDefault
            End If
        Case "PROPERTIES"
            If pCPermiso.GotPermission(PERMISO_LISTA_PRECIO_MODIFY) Then
                ...
                Set ListaPrecio = New ListaPrecio
                ListaPrecio.IDListaPrecio = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
            ...
            End If
        Case "DELETE"
            If pCPermiso.GotPermission(PERMISO_LISTA_PRECIO_DELETE) Then
                If lvwData.SelectedItem Is Nothing Then
                    MsgBox "No hay ningún Item seleccionado.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
                End If
                If MsgBox("¿Desea eliminar la Lista de Precio seleccionada?", vbQuestion + vbYesNo, App.Title) = vbYes Then
                    Set ListaPrecio = New ListaPrecio
                    ListaPrecio.IDListaPrecio = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                    If Not ListaPrecio.Load() Then
                        lvwData.SetFocus
        ...
        Case "SELECT"
            ...
            If FormIndex >= 0 Then
                If lvwData.SelectedItem Is Nothing Then
                    ...
                    Exit Sub
                End If
                If Not GetBooleanValueFromString(lvwData.SelectedItem.SubItems(1)) Then
                    MsgBox "No puede seleccionar este Item ya que está inactivo.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
                End If
                Screen.MousePointer = vbHourglass
                Forms(FormIndex).LugarSelected Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
        ...
        Case "DETAIL"
            If pCPermiso.GotPermission(PERMISO_LISTA_PRECIO_DETALLE) Then
                If lvwData.SelectedItem Is Nothing Then
                    MsgBox "No hay ningún Item seleccionado.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
                End If
                frmListaPrecioDetalle.LoadDataAndShow Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
            End If
    End Select
End Sub
