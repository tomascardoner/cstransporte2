...
Begin VB.Form frmPersonaAlarmaGrupo 
   Caption         =   "Grupos de Alarmas de Personas"
   ClientHeight    =   5955
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   7410
   Icon            =   "PersonaAlarmaGrupo.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MDIChild        =   -1  'True
   ...
   End
End
Attribute VB_Name = "frmPersonaAlarmaGrupo"
...
Public Sub FillListView(ByVal IDPersonaAlarmaGrupo As Long)
    ...
    If IDPersonaAlarmaGrupo = 0 Then
        If Not lvwData.SelectedItem Is Nothing Then
            KeySave = lvwData.SelectedItem.Key
        End If
    Else
        KeySave = KEY_STRINGER & IDPersonaAlarmaGrupo
    End If
    
    SQL_Where = ""
    
    If cboFilterActivo.ListIndex > 0 Then
        SQL_Where = SQL_Where & IIf(SQL_Where = "", " WHERE ", " AND ") & "PersonaAlarmaGrupo.Activo = " & IIf(cboFilterActivo.ListIndex = 1, 1, 0)
    End If
    ...
    recData.Source = "SELECT PersonaAlarmaGrupo.IDPersonaAlarmaGrupo, PersonaAlarmaGrupo.Nombre, PersonaAlarmaGrupo.Activo FROM PersonaAlarmaGrupo" & SQL_Where
    recData.Open , pDatabase.Connection, adOpenForwardOnly, adLockReadOnly, adCmdText
    
    With recData
        If Not .EOF Then
            Do While Not .EOF
                Set ListItem = lvwData.ListItems.Add(, KEY_STRINGER & .Fields("IDPersonaAlarmaGrupo").Value, .Fields("Nombre").Value)
...
ErrorHandler:
    ShowErrorMessage "Forms.PersonaAlarmaGrupo.FillListView", "Error al obtener la lista de Grupos de Alarmas de Personas."
End Sub
...
Private Sub Form_Load()
    ...
    pParametro.GetCoolBarSettings "PersonaAlarmaGrupo", cbrMain
    pParametro.GetListViewSettings "PersonaAlarmaGrupo", lvwData
    lvwData.ColumnHeaders(lvwData.SortKey + 1).Icon = lvwData.SortOrder + 1
    tlbPin.Buttons("PIN").Value = pParametro.Usuario_LeerNumero("PersonaAlarmaGrupo_Pin", tlbPin.Buttons("PIN").Value)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    pParametro.SaveCoolBarSettings "PersonaAlarmaGrupo", cbrMain
    pParametro.SaveListViewSettings "PersonaAlarmaGrupo", lvwData
    pParametro.Usuario_GuardarNumero "PersonaAlarmaGrupo_Pin", tlbPin.Buttons("PIN").Value
End Sub
...
Private Sub tlbMain_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Dim PersonaAlarmaGrupo As PersonaAlarmaGrupo
    
    Select Case Button.Key
        Case "NEW"
            If pCPermiso.GotPermission(PERMISO_PERSONA_ALARMA_GRUPO_ADD) Then
                Screen.MousePointer = vbHourglass
                
                Set PersonaAlarmaGrupo = New PersonaAlarmaGrupo
                frmPersonaAlarmaGrupoPropiedad.LoadDataAndShow Me, PersonaAlarmaGrupo
                Set PersonaAlarmaGrupo = Nothing
                
                Screen.MousePointer = vbDefault
            End If
        Case "PROPERTIES"
            If pCPermiso.GotPermission(PERMISO_PERSONA_ALARMA_GRUPO_MODIFY) Then
                ...
                Set PersonaAlarmaGrupo = New PersonaAlarmaGrupo
                PersonaAlarmaGrupo.IDPersonaAlarmaGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                If PersonaAlarmaGrupo.Load() Then
                    frmPersonaAlarmaGrupoPropiedad.LoadDataAndShow Me, PersonaAlarmaGrupo
                End If
                Set PersonaAlarmaGrupo = Nothing
            ...
            End If
        Case "DELETE"
            If pCPermiso.GotPermission(PERMISO_PERSONA_ALARMA_GRUPO_DELETE) Then
                If lvwData.SelectedItem Is Nothing Then
                    MsgBox "No hay ningún Item seleccionado.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
                End If
                If MsgBox("¿Desea eliminar el Grupo de Alarmas de Personas seleccionado?", vbQuestion + vbYesNo, App.Title) = vbYes Then
                    Set PersonaAlarmaGrupo = New PersonaAlarmaGrupo
                    PersonaAlarmaGrupo.IDPersonaAlarmaGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                    If PersonaAlarmaGrupo.Load() Then
                        Call PersonaAlarmaGrupo.Delete
                    End If
                    Set PersonaAlarmaGrupo = Nothing
        ...
        Case "SELECT"
            ...
            If FormIndex >= 0 Then
                ...
                Set PersonaAlarmaGrupo = New PersonaAlarmaGrupo
                PersonaAlarmaGrupo.IDPersonaAlarmaGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                If Not PersonaAlarmaGrupo.Load() Then
                    Set PersonaAlarmaGrupo = Nothing
                    Exit Sub
                End If
                If Not PersonaAlarmaGrupo.Activo Then
                ...
                End If
                Screen.MousePointer = vbHourglass
                Forms(FormIndex).PersonaAlarmaGrupoSelected PersonaAlarmaGrupo.IDPersonaAlarmaGrupo
                Set PersonaAlarmaGrupo = Nothing
