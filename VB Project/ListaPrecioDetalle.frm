...
Begin VB.Form frmListaPrecioDetalle 
   ...
   Begin TrueOleDBGrid80.TDBGrid tdbgrdData 
      ...
      _StyleDefs(60)  =   "Splits(0).Columns(5).Style:id=16,.parent=43,.alignment=1"
      _StyleDefs(61)  =   "Splits(0).Columns(5).HeadingStyle:id=13,.parent=44,.alignment=2"
      _StyleDefs(62)  =   "Splits(0).Columns(5).FooterStyle:id=14,.parent=45"
      _StyleDefs(63)  =   "Splits(0).Columns(5).EditorStyle:id=15,.parent=47"
...
Public Function FillListView(ByVal IDListaPrecio As Long) As Boolean
    ...
    tdbgrdData.Columns("Importe Web").Locked = False
...
Private Sub tdbgrdData_KeyPress(KeyAscii As Integer)
    ...
    Select Case tdbgrdData.Col
        Case 4
            CampoNombre = "Importe"
        Case 5
            CampoNombre = "Importe Web"
        Case Else
            CampoNombre = ""
    End Select
    ...
    If KeyAscii = Asc(pRegionalSettings.CurrencyDecimalSymbol) Then
        If InStr(1, tdbgrdData.Columns(CampoNombre).value, pRegionalSettings.CurrencyDecimalSymbol) > 0 Then
            KeyAscii = 0
        End If
    End If
    If KeyAscii = Asc(pRegionalSettings.CurrencyCurrencySymbol) Then
        If InStr(1, tdbgrdData.Columns(CampoNombre).value, pRegionalSettings.CurrencyCurrencySymbol) > 0 Then
            KeyAscii = 0
        End If
    End If
End Sub
...
Private Sub tdbgrdData_UnboundColumnFetch(Bookmark As Variant, ByVal Col As Integer, value As Variant)
    ...
    If Col = 4 Or Col = 5 Then
        ...
        value = mData(Col, mClonedRecordset.AbsolutePosition - 1)
    End If
End Sub
...
Private Sub tdbgrdData_BeforeColUpdate(ByVal ColIndex As Integer, OldValue As Variant, Cancel As Integer)
    ...
    If Not mUpdating Then
        If Trim(tdbgrdData.Columns(CampoNombre).value) <> "" Then
            If Not IsNumeric(tdbgrdData.Columns(CampoNombre).value) Then
                MsgBox "El " & CampoNombre & " debe ser un valor numérico.", vbExclamation, App.Title
                mUpdating = True
                tdbgrdData.Columns(CampoNombre).value = OldValue
                mUpdating = True
                tdbgrdData.SetFocus
                Exit Sub
            End If
...
Private Sub tdbgrdData_AfterColUpdate(ByVal ColIndex As Integer)
    ...
    ' Leo la el detalle de la lista de precios desde la base de datos
    ...
    If Not ListaPrecioDetalle.Load() Then
        Set ListaPrecioDetalle = Nothing
        Exit Sub
    End If
    
    ' Si están vacíos los dos importes, elimino el registro si es que existe
    If Trim(tdbgrdData.Columns("Importe").value) = "" And Trim(tdbgrdData.Columns("Importe Web").value) = "" Then
        If ListaPrecioDetalle.NoMatch Then
            Set recData = tdbgrdData.DataSource
            mData(4, recData.AbsolutePosition - 1) = ""
            mData(5, recData.AbsolutePosition - 1) = ""
            Set recData = Nothing
        Else
            ...
            Else
                ...
                Select Case ColIndex
                    Case 4
                        tdbgrdData.Columns(CampoNombre).value = ListaPrecioDetalle.Importe
                    Case 5
                        tdbgrdData.Columns(CampoNombre).value = ListaPrecioDetalle.ImporteWeb
                End Select
                mUpdating = False
            End If
        End If
    ...
    ' Se modificó un importe, así que lo actualizo en la base de datos
    Select Case ColIndex
        Case 4
            ImporteSave = ListaPrecioDetalle.Importe
            If Trim(tdbgrdData.Columns(CampoNombre).value) = "" Then
                ListaPrecioDetalle.Importe = 0
            Else
                ListaPrecioDetalle.Importe = CCur(tdbgrdData.Columns(CampoNombre).value)
            End If
        Case 5
            ImporteSave = ListaPrecioDetalle.ImporteWeb
            If Trim(tdbgrdData.Columns(CampoNombre).value) = "" Then
                ListaPrecioDetalle.ImporteWeb = -1
            Else
                ListaPrecioDetalle.ImporteWeb = CCur(tdbgrdData.Columns(CampoNombre).value)
            End If
    End Select
    If ListaPrecioDetalle.NoMatch Then
        ' No existe el registro en la base de datos, lo agrego
        If ListaPrecioDetalle.AddNew() Then
            Set recData = tdbgrdData.DataSource
            Select Case ColIndex
                Case 4
                    mData(ColIndex, recData.AbsolutePosition - 1) = ListaPrecioDetalle.Importe
                Case 5
                    mData(ColIndex, recData.AbsolutePosition - 1) = ListaPrecioDetalle.ImporteWeb
            End Select
            Set recData = Nothing
        Else
            mUpdating = True
            tdbgrdData.Columns(CampoNombre).value = ""
            mUpdating = False
        End If
    Else
        ' Ya existe el registro en la base de datos, lo actualizo
        If ListaPrecioDetalle.Update() Then
            ...
            Select Case ColIndex
                Case 4
                    mData(ColIndex, recData.AbsolutePosition - 1) = ListaPrecioDetalle.Importe
                Case 5
                    mData(ColIndex, recData.AbsolutePosition - 1) = IIf(ListaPrecioDetalle.ImporteWeb = -1, "", ListaPrecioDetalle.ImporteWeb)
            End Select
            Set recData = Nothing
        Else
            mUpdating = True
            tdbgrdData.Columns(CampoNombre).value = ImporteSave
            mUpdating = False
        End If
    End If
    tdbgrdData.Columns(4).Locked = False
    tdbgrdData.Columns(5).Locked = False
    Set ListaPrecioDetalle = Nothing
End Sub
...
Private Sub ResizeControls(ByVal CoolBarHeight As Single)
    ...
    tdbgrdData.Columns("LugarGrupoOrigen").Width = (tdbgrdData.Width - SCROLLBAR_WIDTH - tdbgrdData.Columns("Importe").Width - tdbgrdData.Columns("ImporteWeb").Width) / 2
...
Private Sub tlbMain_ButtonClick(ByVal Button As MSComctlLib.Button)
    Select Case Button.Key
        Case "PRINT"
            tdbgrdData.Columns("LugarGrupoOrigen").Width = 3500
            tdbgrdData.Columns("LugarGrupoDestino").Width = 3500
