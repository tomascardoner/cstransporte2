...
Public Sub FillListView(ByVal IDRuta As String)
    ...
    SQL_Where = " WHERE Ruta.IDRuta <> '" & ReplaceQuote(pParametro.Ruta_ID_Otra) & "' AND Ruta.IDRuta <> '" & ReplaceQuote(pParametro.Ruta_Paquete_ID) & "'"
    
    If pCPermiso.RutaWhere <> "" Then
        SQL_Where = SQL_Where & IIf(SQL_Where = "", " WHERE ", " AND ") & Replace(pCPermiso.RutaWhere, "%TABLENAME%", "Ruta")
    End If
    
    If cboFilterActivo.ListIndex > 0 Then
        SQL_Where = SQL_Where & IIf(SQL_Where = "", " WHERE ", " AND ") & "Ruta.Activo = " & IIf(cboFilterActivo.ListIndex = 1, 1, 0)
    End If
    ...
    recData.Source = "SELECT Ruta.IDRuta, Ruta.Nombre, Origen.Nombre AS Origen, Destino.Nombre AS Destino, RutaGrupo.Nombre AS Grupo, Ruta.Activo FROM ((Ruta INNER JOIN Lugar AS Origen ON Ruta.IDOrigen = Origen.IDLugar) INNER JOIN Lugar AS Destino ON Ruta.IDDestino = Destino.IDLugar) INNER JOIN RutaGrupo ON Ruta.IDrutaGrupo = RutaGrupo.IDRutaGrupo" & SQL_Where
    ...
    With recData
        If Not .EOF Then
            Do While Not .EOF
                ...
                ListItem.SubItems(2) = .Fields("Origen").Value
                ListItem.SubItems(3) = .Fields("Destino").Value
                ListItem.SubItems(4) = .Fields("Grupo").Value
...
ErrorHandler:
    ShowErrorMessage "Forms.Ruta.FillListView", "Error al obtener la Lista de Rutas."
End Sub
...
Private Sub Form_Load()
    ...
    pParametro.GetCoolBarSettings "Ruta", cbrMain
    pParametro.GetListViewSettings "Ruta", lvwData
    lvwData.ColumnHeaders(lvwData.SortKey + 1).Icon = lvwData.SortOrder + 1
    tlbPin.Buttons("PIN").Value = pParametro.Usuario_LeerNumero("Ruta_Pin", tlbPin.Buttons("PIN").Value)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    pParametro.SaveCoolBarSettings "Ruta", cbrMain
    pParametro.SaveListViewSettings "Ruta", lvwData
    pParametro.Usuario_GuardarNumero "Ruta_Pin", tlbPin.Buttons("PIN").Value
    Set frmRuta = Nothing
End Sub
...
Private Sub tlbMain_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Select Case Button.Key
        Case "NEW"
            If pCPermiso.GotPermission(PERMISO_RUTA_ADD) Then
                Screen.MousePointer = vbHourglass
                
                Set Ruta = New Ruta
                frmRutaPropiedad.LoadDataAndShow Me, Ruta
                Set Ruta = Nothing
                
                Screen.MousePointer = vbDefault
            End If
        Case "PROPERTIES"
            If pCPermiso.GotPermission(PERMISO_RUTA_MODIFY) Then
                ...
                Set Ruta = New Ruta
                Ruta.IDRuta = Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1)
                If Ruta.Load() Then
                    frmRutaPropiedad.LoadDataAndShow Me, Ruta
                Else
                    lvwData.SetFocus
            ...
            End If
        Case "DELETE"
            If pCPermiso.GotPermission(PERMISO_RUTA_DELETE) Then
                If lvwData.SelectedItem Is Nothing Then
                    MsgBox "No hay ningún Item seleccionado.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
                End If
                If MsgBox("¿Desea eliminar la Ruta seleccionada?", vbQuestion + vbYesNo, App.Title) = vbYes Then
                    Set Ruta = New Ruta
                    Ruta.IDRuta = Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1)
                    If Ruta.Load() Then
                        Call Ruta.Delete
        ...
        Case "SELECT"
            ...
            If FormIndex >= 0 Then
                ...
                End If
                Screen.MousePointer = vbHourglass
                Forms(FormIndex).RutaSelected Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1)
