...
Begin VB.Form frmCuentaCorrienteGrupo 
   Caption         =   "Grupos de Cuenta Corriente"
   ClientHeight    =   5955
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   7410
   Icon            =   "CuentaCorrienteGrupo.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MDIChild        =   -1  'True
   ...
   End
End
Attribute VB_Name = "frmCuentaCorrienteGrupo"
...
Public Sub FillListView(ByVal IDCuentaCorrienteGrupo As Long)
    ...
    If IDCuentaCorrienteGrupo = 0 Then
        If Not lvwData.SelectedItem Is Nothing Then
            KeySave = lvwData.SelectedItem.Key
        End If
    Else
        KeySave = KEY_STRINGER & IDCuentaCorrienteGrupo
    End If
    
    SQL_Where = ""
    
    If cboFilterActivo.ListIndex > 0 Then
        SQL_Where = SQL_Where & IIf(SQL_Where = "", " WHERE ", " AND ") & "CuentaCorrienteGrupo.Activo = " & IIf(cboFilterActivo.ListIndex = 1, 1, 0)
    End If
    ...
    recData.Source = "SELECT CuentaCorrienteGrupo.IDCuentaCorrienteGrupo, CuentaCorrienteGrupo.Nombre, CuentaCorrienteGrupo.Activo FROM CuentaCorrienteGrupo" & SQL_Where
    recData.Open , pDatabase.Connection, adOpenForwardOnly, adLockReadOnly, adCmdText
    
    With recData
        If Not .EOF Then
            Do While Not .EOF
                Set ListItem = lvwData.ListItems.Add(, KEY_STRINGER & .Fields("IDCuentaCorrienteGrupo").Value, .Fields("Nombre").Value)
...
ErrorHandler:
    ShowErrorMessage "Forms.CuentaCorrienteGrupo.FillListView", "Error al obtener la lista de Grupos de Cuenta Corriente."
End Sub
...
Private Sub Form_Load()
    ...
    pParametro.GetCoolBarSettings "CuentaCorrienteGrupo", cbrMain
    pParametro.GetListViewSettings "CuentaCorrienteGrupo", lvwData
    lvwData.ColumnHeaders(lvwData.SortKey + 1).Icon = lvwData.SortOrder + 1
    tlbPin.Buttons("PIN").Value = pParametro.Usuario_LeerNumero("CuentaCorrienteGrupo_Pin", tlbPin.Buttons("PIN").Value)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    pParametro.SaveCoolBarSettings "CuentaCorrienteGrupo", cbrMain
    pParametro.SaveListViewSettings "CuentaCorrienteGrupo", lvwData
    pParametro.Usuario_GuardarNumero "CuentaCorrienteGrupo_Pin", tlbPin.Buttons("PIN").Value
End Sub
...
Private Sub tlbMain_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Dim CuentaCorrienteGrupo As CuentaCorrienteGrupo
    
    Select Case Button.Key
        Case "NEW"
            If pCPermiso.GotPermission(PERMISO_CUENTA_CORRIENTE_GRUPO_ADD) Then
                Screen.MousePointer = vbHourglass
                
                Set CuentaCorrienteGrupo = New CuentaCorrienteGrupo
                frmCuentaCorrienteGrupoPropiedad.LoadDataAndShow Me, CuentaCorrienteGrupo
                Set CuentaCorrienteGrupo = Nothing
                
                Screen.MousePointer = vbDefault
            End If
        Case "PROPERTIES"
            If pCPermiso.GotPermission(PERMISO_CUENTA_CORRIENTE_GRUPO_MODIFY) Then
                ...
                Set CuentaCorrienteGrupo = New CuentaCorrienteGrupo
                CuentaCorrienteGrupo.IDCuentaCorrienteGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                If CuentaCorrienteGrupo.Load() Then
                    frmCuentaCorrienteGrupoPropiedad.LoadDataAndShow Me, CuentaCorrienteGrupo
                End If
                Set CuentaCorrienteGrupo = Nothing
            ...
            End If
        Case "DELETE"
            If pCPermiso.GotPermission(PERMISO_CUENTA_CORRIENTE_GRUPO_DELETE) Then
                If lvwData.SelectedItem Is Nothing Then
                    MsgBox "No hay ningún Item seleccionado.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
                End If
                If MsgBox("¿Desea eliminar el Grupo de Cuenta Corriente seleccionado?", vbQuestion + vbYesNo, App.Title) = vbYes Then
                    Set CuentaCorrienteGrupo = New CuentaCorrienteGrupo
                    CuentaCorrienteGrupo.IDCuentaCorrienteGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                    If CuentaCorrienteGrupo.Load() Then
                        Call CuentaCorrienteGrupo.Delete
                    End If
                    Set CuentaCorrienteGrupo = Nothing
        ...
        Case "SELECT"
            ...
            If FormIndex >= 0 Then
                ...
                Set CuentaCorrienteGrupo = New CuentaCorrienteGrupo
                CuentaCorrienteGrupo.IDCuentaCorrienteGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                If Not CuentaCorrienteGrupo.Load() Then
                    Set CuentaCorrienteGrupo = Nothing
                    Exit Sub
                End If
                If Not CuentaCorrienteGrupo.Activo Then
                ...
                End If
                Screen.MousePointer = vbHourglass
                Forms(FormIndex).CuentaCorrienteGrupoSelected CuentaCorrienteGrupo.IDCuentaCorrienteGrupo
                Set CuentaCorrienteGrupo = Nothing
