...
Begin VB.Form frmLugarGrupo 
   Caption         =   "Grupos de Lugares"
   ClientHeight    =   5955
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   7410
   Icon            =   "LugarGrupo.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MDIChild        =   -1  'True
   ...
   End
End
Attribute VB_Name = "frmLugarGrupo"
...
Public Sub FillListView(ByVal IDLugarGrupo As Long)
    ...
    If IDLugarGrupo = 0 Then
        If Not lvwData.SelectedItem Is Nothing Then
            KeySave = lvwData.SelectedItem.Key
        End If
    Else
        KeySave = KEY_STRINGER & IDLugarGrupo
    End If
    
    SQL_Where = " WHERE LugarGrupo.IDLugarGrupo <> " & pParametro.LugarGrupo_ID_Otro
    
    If cboFilterActivo.ListIndex > 0 Then
        SQL_Where = SQL_Where & IIf(SQL_Where = "", " WHERE ", " AND ") & "LugarGrupo.Activo = " & IIf(cboFilterActivo.ListIndex = 1, 1, 0)
    End If
    ...
    recData.Source = "SELECT LugarGrupo.IDLugarGrupo, LugarGrupo.Nombre, LugarGrupo.Activo FROM LugarGrupo" & SQL_Where
    recData.Open , pDatabase.Connection, adOpenForwardOnly, adLockReadOnly, adCmdText
    
    With recData
        If Not .EOF Then
            Do While Not .EOF
                Set ListItem = lvwData.ListItems.Add(, KEY_STRINGER & .Fields("IDLugarGrupo").Value, .Fields("Nombre").Value)
...
ErrorHandler:
    ShowErrorMessage "Forms.LugarGrupo.FillListView", "Error al obtener la lista de Grupos de Lugares"
End Sub
...
Private Sub Form_Load()
    ...
    pParametro.GetCoolBarSettings "LugarGrupo", cbrMain
    pParametro.GetListViewSettings "LugarGrupo", lvwData
    lvwData.ColumnHeaders(lvwData.SortKey + 1).Icon = lvwData.SortOrder + 1
    tlbPin.Buttons("PIN").Value = pParametro.Usuario_LeerNumero("LugarGrupo_Pin", tlbPin.Buttons("PIN").Value)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    pParametro.SaveCoolBarSettings "LugarGrupo", cbrMain
    pParametro.SaveListViewSettings "LugarGrupo", lvwData
    pParametro.Usuario_GuardarNumero "LugarGrupo_Pin", tlbPin.Buttons("PIN").Value
End Sub
...
Private Sub tlbMain_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Dim LugarGrupo As LugarGrupo
    
    Select Case Button.Key
        Case "NEW"
            If pCPermiso.GotPermission(PERMISO_LUGAR_GRUPO_ADD) Then
                Screen.MousePointer = vbHourglass
                
                Set LugarGrupo = New LugarGrupo
                frmLugarGrupoPropiedad.LoadDataAndShow Me, LugarGrupo
                Set LugarGrupo = Nothing
                
                Screen.MousePointer = vbDefault
            End If
        Case "PROPERTIES"
            If pCPermiso.GotPermission(PERMISO_LUGAR_GRUPO_MODIFY) Then
                ...
                Set LugarGrupo = New LugarGrupo
                LugarGrupo.IDLugarGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                If LugarGrupo.Load() Then
                    frmLugarGrupoPropiedad.LoadDataAndShow Me, LugarGrupo
                End If
                Set LugarGrupo = Nothing
            ...
            End If
        Case "DELETE"
            If pCPermiso.GotPermission(PERMISO_LUGAR_GRUPO_DELETE) Then
                If lvwData.SelectedItem Is Nothing Then
                    MsgBox "No hay ningún Item seleccionado.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
                End If
                If MsgBox("¿Desea eliminar el Grupo de Lugares seleccionado?", vbQuestion + vbYesNo, App.Title) = vbYes Then
                    Set LugarGrupo = New LugarGrupo
                    LugarGrupo.IDLugarGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                    If LugarGrupo.Load() Then
                        Call LugarGrupo.Delete
                    End If
                    Set LugarGrupo = Nothing
        ...
        Case "SELECT"
            ...
            If FormIndex >= 0 Then
                ...
                Set LugarGrupo = New LugarGrupo
                LugarGrupo.IDLugarGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                If Not LugarGrupo.Load() Then
                    Set LugarGrupo = Nothing
                    Exit Sub
                End If
                If Not LugarGrupo.Activo Then
                ...
                End If
                Screen.MousePointer = vbHourglass
                Forms(FormIndex).LugarGrupoSelected LugarGrupo.IDLugarGrupo
                Set LugarGrupo = Nothing
