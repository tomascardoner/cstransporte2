...
ErrorHandler:
    ShowErrorMessage "Forms.Vehiculo.FillListView", "Error al obtener la lista de los Vehículos."
End Sub
...
Private Sub Form_Load()
    ...
    pParametro.GetCoolBarSettings "Vehiculo", cbrMain
    pParametro.GetListViewSettings "Vehiculo", lvwData
    lvwData.ColumnHeaders(lvwData.SortKey + 1).Icon = lvwData.SortOrder + 1
    tlbPin.Buttons("PIN").Value = pParametro.Usuario_LeerNumero("Vehiculo_Pin", tlbPin.Buttons("PIN").Value)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    pParametro.SaveCoolBarSettings "Vehiculo", cbrMain
    pParametro.SaveListViewSettings "Vehiculo", lvwData
    pParametro.Usuario_GuardarNumero "Vehiculo_Pin", tlbPin.Buttons("PIN").Value
    Set frmVehiculo = Nothing
End Sub
...
Private Sub tlbMain_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Select Case Button.Key
        Case "NEW"
            If pCPermiso.GotPermission(PERMISO_VEHICULO_ADD) Then
                Screen.MousePointer = vbHourglass
                frmVehiculoPropiedad.LoadDataAndShow Me, Vehiculo
                Screen.MousePointer = vbDefault
            End If
        Case "PROPERTIES"
            If pCPermiso.GotPermission(PERMISO_VEHICULO_MODIFY) Then
                ...
                Vehiculo.IDVehiculo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
            ...
            End If
        Case "DELETE"
            If pCPermiso.GotPermission(PERMISO_VEHICULO_DELETE) Then
                If lvwData.SelectedItem Is Nothing Then
                    MsgBox "No hay ningún Item seleccionado.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
                End If
                If MsgBox("¿Desea eliminar el Vehículo seleccionado?", vbExclamation + vbYesNo, App.Title) = vbYes Then
        ...
        Case "SELECT"
            ...
            If FormIndex >= 0 Then
                ...
                End If
                Screen.MousePointer = vbHourglass
                Forms(FormIndex).VehiculoSelected Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
