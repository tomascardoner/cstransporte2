...
Public Sub FillListView(ByVal IDUsuarioGrupo As Long)
    ...
    If IDUsuarioGrupo = 0 Then
        ...
        End If
    Else
        KeySave = KEY_STRINGER & IDUsuarioGrupo
    End If
    
    SQL_Where = " WHERE IDUsuarioGrupo <> " & USUARIO_GRUPO_ID_ADMINISTRADORES
    ...
    recData.Source = "SELECT IDUsuarioGrupo, Nombre, Activo FROM UsuarioGrupo" & SQL_Where
    recData.Open , pDatabase.Connection, adOpenForwardOnly, adLockReadOnly, adCmdText
    
    With recData
        If Not .EOF Then
            Do While Not .EOF
                Set ListItem = lvwData.ListItems.Add(, KEY_STRINGER & .Fields("IDUsuarioGrupo").Value, .Fields("Nombre").Value)
...
ErrorHandler:
    ShowErrorMessage "Forms.UsuarioGrupo.FillListView", "Error al obtener la Lista de Grupos de Usuarios."
End Sub
...
Private Sub Form_Load()
    ...
    tlbMain.Buttons("PERMISSION").Image = "PERMISSION"
    '//////////////////////////////////////////////////////////
    ...
    pParametro.GetCoolBarSettings "UsuarioGrupo", cbrMain
    pParametro.GetListViewSettings "UsuarioGrupo", lvwData
    lvwData.ColumnHeaders(lvwData.SortKey + 1).Icon = lvwData.SortOrder + 1
    tlbPin.Buttons("PIN").Value = pParametro.Usuario_LeerNumero("UsuarioGrupo_Pin", tlbPin.Buttons("PIN").Value)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    pParametro.SaveCoolBarSettings "UsuarioGrupo", cbrMain
    pParametro.SaveListViewSettings "UsuarioGrupo", lvwData
    pParametro.Usuario_GuardarNumero "UsuarioGrupo_Pin", tlbPin.Buttons("PIN").Value
End Sub
...
Private Sub tlbMain_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Select Case Button.Key
        Case "NEW"
            If pCPermiso.GotPermission(PERMISO_USUARIO_GRUPO_ADD) Then
                Screen.MousePointer = vbHourglass
                
                Set UsuarioGrupo = New UsuarioGrupo
                frmUsuarioGrupoPropiedad.LoadDataAndShow Me, UsuarioGrupo
                Set UsuarioGrupo = Nothing
                
                Screen.MousePointer = vbDefault
            End If
        Case "PROPERTIES"
            If pCPermiso.GotPermission(PERMISO_USUARIO_GRUPO_MODIFY) Then
                ...
                Set UsuarioGrupo = New UsuarioGrupo
                UsuarioGrupo.IDUsuarioGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                If UsuarioGrupo.Load() Then
                    frmUsuarioGrupoPropiedad.LoadDataAndShow Me, UsuarioGrupo
                Else
                    lvwData.SetFocus
                End If
            ...
            End If
        Case "DELETE"
            If pCPermiso.GotPermission(PERMISO_USUARIO_GRUPO_DELETE) Then
                If lvwData.SelectedItem Is Nothing Then
                    MsgBox "No hay ningún Item seleccionado.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
                End If
                If MsgBox("¿Desea eliminar el Grupo de Usuarios seleccionado?", vbQuestion + vbYesNo, App.Title) = vbYes Then
                    Set UsuarioGrupo = New UsuarioGrupo
                    UsuarioGrupo.IDUsuarioGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                    If UsuarioGrupo.Load() Then
                        Call UsuarioGrupo.Delete
                    End If
                    Set UsuarioGrupo = Nothing
                    lvwData.SetFocus
                End If
        ...
        Case "SELECT"
            ...
            If FormIndex >= 0 Then
                ...
                End If
                Screen.MousePointer = vbHourglass
                Forms(FormIndex).UsuarioGrupoSelected Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
        ...
        Case "PERMISSION"
            If pCPermiso.GotPermission(PERMISO_USUARIO_GRUPO_PERMISSION) Then
                If lvwData.SelectedItem Is Nothing Then
                    MsgBox "No hay ningún Item seleccionado.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
                End If
                Screen.MousePointer = vbHourglass
                frmUsuarioGrupoPermiso.LoadDataAndShow Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1)
                Screen.MousePointer = vbDefault
            End If
    End Select
End Sub
