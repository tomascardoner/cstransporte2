...
Begin VB.Form frmVehiculoMantenimientoGrupo 
   Caption         =   "Grupos de Mantenimiento de Vehículos"
   ClientHeight    =   5955
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   7410
   Icon            =   "VehiculoMantenimientoGrupo.frx":0000
   KeyPreview      =   -1  'True
   LinkTopic       =   "Form1"
   LockControls    =   -1  'True
   MDIChild        =   -1  'True
   ...
   End
End
Attribute VB_Name = "frmVehiculoMantenimientoGrupo"
...
Public Sub FillListView(ByVal IDVehiculoMantenimientoGrupo As Long)
    ...
    If IDVehiculoMantenimientoGrupo = 0 Then
        If Not lvwData.SelectedItem Is Nothing Then
            KeySave = lvwData.SelectedItem.Key
        End If
    Else
        KeySave = KEY_STRINGER & IDVehiculoMantenimientoGrupo
    End If
    
    SQL_Where = ""
    
    If cboFilterActivo.ListIndex > 0 Then
        SQL_Where = SQL_Where & IIf(SQL_Where = "", " WHERE ", " AND ") & "VehiculoMantenimientoGrupo.Activo = " & IIf(cboFilterActivo.ListIndex = 1, 1, 0)
    End If
    ...
    recData.Source = "SELECT VehiculoMantenimientoGrupo.IDVehiculoMantenimientoGrupo, VehiculoMantenimientoGrupo.Nombre, VehiculoMantenimientoGrupo.Activo FROM VehiculoMantenimientoGrupo" & SQL_Where
    recData.Open , pDatabase.Connection, adOpenForwardOnly, adLockReadOnly, adCmdText
    
    With recData
        If Not .EOF Then
            Do While Not .EOF
                Set ListItem = lvwData.ListItems.Add(, KEY_STRINGER & .Fields("IDVehiculoMantenimientoGrupo").Value, .Fields("Nombre").Value)
...
ErrorHandler:
    ShowErrorMessage "Forms.VehiculoMantenimientoGrupo.FillListView", "Error al obtener la lista de Grupos de Mantenimiento de Vehículos."
End Sub
...
Private Sub Form_Load()
    ...
    pParametro.GetCoolBarSettings "VehiculoMantenimientoGrupo", cbrMain
    pParametro.GetListViewSettings "VehiculoMantenimientoGrupo", lvwData
    lvwData.ColumnHeaders(lvwData.SortKey + 1).Icon = lvwData.SortOrder + 1
    tlbPin.Buttons("PIN").Value = pParametro.Usuario_LeerNumero("VehiculoMantenimientoGrupo_Pin", tlbPin.Buttons("PIN").Value)
...
Private Sub Form_Unload(Cancel As Integer)
    ...
    pParametro.SaveCoolBarSettings "VehiculoMantenimientoGrupo", cbrMain
    pParametro.SaveListViewSettings "VehiculoMantenimientoGrupo", lvwData
    pParametro.Usuario_GuardarNumero "VehiculoMantenimientoGrupo_Pin", tlbPin.Buttons("PIN").Value
End Sub
...
Private Sub tlbMain_ButtonClick(ByVal Button As MSComctlLib.Button)
    ...
    Dim VehiculoMantenimientoGrupo As VehiculoMantenimientoGrupo
    
    Select Case Button.Key
        Case "NEW"
            If pCPermiso.GotPermission(PERMISO_VEHICULO_MANTENIMIENTO_GRUPO_ADD) Then
                Screen.MousePointer = vbHourglass
                
                Set VehiculoMantenimientoGrupo = New VehiculoMantenimientoGrupo
                frmVehiculoMantenimientoGrupoPropiedad.LoadDataAndShow Me, VehiculoMantenimientoGrupo
                Set VehiculoMantenimientoGrupo = Nothing
                
                Screen.MousePointer = vbDefault
            End If
        Case "PROPERTIES"
            If pCPermiso.GotPermission(PERMISO_VEHICULO_MANTENIMIENTO_GRUPO_MODIFY) Then
                ...
                Set VehiculoMantenimientoGrupo = New VehiculoMantenimientoGrupo
                VehiculoMantenimientoGrupo.IDVehiculoMantenimientoGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                If VehiculoMantenimientoGrupo.Load() Then
                    frmVehiculoMantenimientoGrupoPropiedad.LoadDataAndShow Me, VehiculoMantenimientoGrupo
                End If
                Set VehiculoMantenimientoGrupo = Nothing
            ...
            End If
        Case "DELETE"
            If pCPermiso.GotPermission(PERMISO_VEHICULO_MANTENIMIENTO_GRUPO_DELETE) Then
                If lvwData.SelectedItem Is Nothing Then
                    MsgBox "No hay ningún Item seleccionado.", vbInformation, App.Title
                    lvwData.SetFocus
                    Exit Sub
                End If
                If MsgBox("¿Desea eliminar el Grupo de Mantenimiento de Vehículos seleccionado?", vbQuestion + vbYesNo, App.Title) = vbYes Then
                    Set VehiculoMantenimientoGrupo = New VehiculoMantenimientoGrupo
                    VehiculoMantenimientoGrupo.IDVehiculoMantenimientoGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                    If VehiculoMantenimientoGrupo.Load() Then
                        Call VehiculoMantenimientoGrupo.Delete
                    End If
                    Set VehiculoMantenimientoGrupo = Nothing
        ...
        Case "SELECT"
            ...
            If FormIndex >= 0 Then
                ...
                Set VehiculoMantenimientoGrupo = New VehiculoMantenimientoGrupo
                VehiculoMantenimientoGrupo.IDVehiculoMantenimientoGrupo = Val(Mid(lvwData.SelectedItem.Key, Len(KEY_STRINGER) + 1))
                If Not VehiculoMantenimientoGrupo.Load() Then
                    Set VehiculoMantenimientoGrupo = Nothing
                    Exit Sub
                End If
                If Not VehiculoMantenimientoGrupo.Activo Then
                ...
                End If
                Screen.MousePointer = vbHourglass
                Forms(FormIndex).VehiculoMantenimientoGrupoSelected VehiculoMantenimientoGrupo.IDVehiculoMantenimientoGrupo
                Set VehiculoMantenimientoGrupo = Nothing
